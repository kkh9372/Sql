...
#실습하기 4-1
	...
	SELECT*FROM `MEMBER` WHERE `name` != '김춘추';
	SELECT*FROM `MEMBER` WHERE `name` <> '김춘추';
	SELECT*FROM `MEMBER` WHERE `pos` = '사원' AND `dep` = 101;
	SELECT*FROM `MEMBER` WHERE `pos` = '사원' or `dep` = 101;
...
#1
	SELECT SUM(`sales`) AS `2018년 1월 매출` FROM `SALE`
...
#2
	SELECT 
		...
		FROM `SALE`
	 	WHERE `year`=2019 AND 
		 `month`=2 AND `sales`>= 50000;
	
#3
	SELECT
	 MIN(`sales`) AS `최저 매출`,
	 MAX(`sales`) AS `최고 매출`
	FROM `SALE` WHERE `year` IN(2020);	 
